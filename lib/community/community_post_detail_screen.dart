...
class _CommunityPostDetailScreenState extends State<CommunityPostDetailScreen> {
  ...
  Future<void> _toggleReaction(String type) async {
    ...
    try {
      if (wasActive) {
        await Supabase.instance.client.from('community_post_reactions').delete().match({
          'user_id': user.id,
          'post_id': postId,
          'reaction_type': type,
        });
  ...
  Future<void> _fetchComments() async {
    try {
      ...
      // For each root, build its flattened tree
  ...
  Future<void> _addComment() async {
    ...
    try {
      ...
      final profileResponse = await Supabase.instance.client.from('profiles').select('id, username, avatar_url, is_verified').eq('id', user.id).single();
      
      final fullComment = {
        ...newComment,
        'username': profileResponse['username'],
        'avatar_url': profileResponse['avatar_url'],
        'is_verified': profileResponse['is_verified'] ?? false,
      };
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Text('Community Post', style: GoogleFonts.outfit(fontSize: isSmallScreen ? 16 : 18, fontWeight: FontWeight.w700, color: textColor)),
                  ],
                ),
              ),
              Expanded(
  ...
  Widget _buildWebRightSidebar(bool isDark, Color cardColor, Color borderColor, Color textColor, Color secondaryTextColor, Color accent) {
    return Column(
      ...
      children: [
        _sidebarCard(
          'About Author',
          'Contributing to the Aurbit community since ${_timeAgo(widget.post['created_at'] ?? DateTime.now().toString())}.',
                    ...
                    Column(
                      ...
                      children: [
                        UsernameWithBadge(
                          ...
                          isVerified: widget.post['is_verified'] ?? false,
  ...
  Widget _buildCommentList() {
    ...
    return Column(
      children: _rootComments.map((root) {
        ...
        return CommunityResponseChainWidget(
          root: root,
          replies: replies,
...
// Reuse the same CommunityResponseChainWidget and CommunitySingleCommentWidget from post_detail_screen.dart
class CommunityResponseChainWidget extends StatefulWidget {
  ...
  const CommunityResponseChainWidget({
    super.key,
    required this.root,
  ...
  @override
  State<CommunityResponseChainWidget> createState() => _CommunityResponseChainWidgetState();
}

class _CommunityResponseChainWidgetState extends State<CommunityResponseChainWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        ...visibleReplies.map((reply) => CommunitySingleCommentWidget(
          comment: reply,
          isDark: widget.isDark,
          isReply: true,
          onReplyTap: widget.onReplyTap,
...
class CommunitySingleCommentWidget extends StatelessWidget {
  ...
  const CommunitySingleCommentWidget({
    super.key,
              ...
              if (visualDepth > 0)
               Container(
                 ...
                 child: CustomPaint(
                   ...
                   painter: _CommunityResponseChainPainter(isDark: isDark),
                 ),
               ),

               if (visualDepth > 0) const SizedBox(width: 4),
              _buildAvatar(context, comment['avatar_url'], visualDepth > 0 ? 28 : 32, isDark),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildAvatar(BuildContext context, String? url, double size, bool isDark) {
    return GestureDetector(
      onTap: () {
...
class _CommunityResponseChainPainter extends CustomPainter {
  final bool isDark;

  _CommunityResponseChainPainter({required this.isDark});
